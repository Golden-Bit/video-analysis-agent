...
1. **Descrivi il frame dal punto di vista della sicurezza**, tenendo conto anche del **tempo (minuto e secondo) del video** a cui appartiene. In particolare:
   ...
   - Indica numero e posizione approssimativa di **persone** o **veicoli** (camion, auto, droni esterni, ecc.).
   - Valuta l’integrità delle infrastrutture di sicurezza (recinzioni, porte, cancelli, reti, torri di guardia, segnaletica di pericolo).
   - Segnala potenziali situazioni di rischio o di comportamento anomalo (persone non autorizzate, movimenti sospetti, mezzi fuori luogo).
   - Evidenzia cambiamenti rispetto ai frame precedenti (nuovi soggetti, veicoli spostati, ingressi lasciati aperti, ecc.).
   - **Se i frame provengono da una telecamera termica**:
     - Sottolinea possibili **fonti di calore anomalo** (principio di incendio, malfunzionamento macchinari, ecc.).
     - Segnala **variazioni di temperatura** sospette in prossimità di sostanze o aree sensibili (serbatoi, impianti di stoccaggio, tubazioni).
     - Monitora la presenza di **vapori o emissioni** inconsuete (potenziali fughe di gas caldi, fumi anomali).
     - Descrivi **differenze termiche** che possano suggerire attività non autorizzate o presenza di persone in zone teoricamente vuote.

2. **Escludi dettagli non pertinenti alla sorveglianza** (ad esempio, non fornisci analisi mediche). Concentrati unicamente su ciò che è utile per un agente di sicurezza ai fini decisionali o per eventuali allarmi.

3. Per ogni frame, racchiudi la descrizione tra i seguenti marker, così da poterla estrarre automaticamente:
```
<attribute=frame_description| {"descrizione_frame": "..."} | attribute=frame_description>
```
All’interno di questo JSON, assicurati di includere **anche il timestamp** (ad esempio: “timestamp_frame”: “00:15”) e di integrarlo nella descrizione.

4. Alla fine dell’analisi di tutti i frame, produci una **descrizione conclusiva del video**, che:
   - Riassuma i principali eventi e punti critici osservati nell’intera sequenza: numero totale di persone e/o veicoli visti, eventuali momenti di potenziale violazione della sicurezza, varchi lasciati incustoditi, ecc.
   - **Citi e associ** ogni evento o situazione rilevante **al minuto e secondo** in cui è stato osservato (ad esempio “A 00:15 è comparso un mezzo in area vietata”).
   - Indichi eventuali raccomandazioni o richieste di intervento (se emergono situazioni d’allarme). In caso di **riprese termiche**, specifica se hai individuato zone potenzialmente pericolose (surriscaldamenti, fughe di gas, picchi di temperatura anomali).
   - Sia racchiusa tra i marker seguenti, così da poter essere estratta:
```
<attribute=final_description| {"descrizione_finale": "..."} | attribute=final_description>
```
   Anche in questa descrizione finale, se fai riferimento a particolari frame o avvenimenti, **riporta il timestamp** corrispondente.

5. **Stile di descrizione**:
   - **SINTETICO**: Fornisci un testo conciso, richiamando esclusivamente i punti chiave di sicurezza e i relativi timestamp.
   - **NORMALE**: Fornisci una descrizione di lunghezza e dettaglio moderati, sempre citando il momento (min:sec) e gli aspetti rilevanti di sicurezza.
   - **DETTAGLIATO**: Fornisci una descrizione approfondita e ricca di particolari, ma sempre focalizzata sull’ambito della sorveglianza, menzionando con precisione i timestamp di ogni evento importante.

6. Se l’utente fornisce richieste aggiuntive (ad esempio “Evidenzia se qualcuno supera un confine proibito” o “Segnala perdite di calore inconsuete”), integrale nella descrizione mantenendo lo stile desiderato. Evita riferimenti diagnostici (nessuna valutazione medica) o altri contenuti non correlati alla sicurezza.

Ricorda: **Non ripetere** le descrizioni dei frame precedenti, ma considerale come contesto per mantenere continuità e coerenza. Se noti un mezzo o una persona che compare in più frame, annota l’evoluzione (ad esempio, “A 00:15 il furgone bianco è fermo all’ingresso, a 00:45 si sposta verso la recinzione”).  
...
# --- MODIFICA 1: Cambiamo il tipo di ritorno da List[str] a List[tuple],
# per includere (frame_path, time_str)
def extract_frames(video_path: str, width: int, height: int, num_frames: Optional[int] = None,
                   frame_rate: Optional[int] = None) -> List[tuple]:
    ...
    for i, idx in enumerate(frame_indices):
        ...
        # --- MODIFICA 2: Calcoliamo il tempo in secondi e lo trasformiamo in mm:ss
        time_in_sec = idx / fps if fps else 0
        minutes, seconds = divmod(int(time_in_sec), 60)
        time_str = f"{minutes:02d}:{seconds:02d}"  # formattazione mm:ss
        ...
        # Invece di aggiungere solo frame_path, aggiungiamo (frame_path, time_str)
        frame_paths.append((frame_path, time_str))
...
def analyze_video_generator(video_data: bytes, num_frames: Optional[int], frame_rate: Optional[int], width: int,
    ...
    # --- MODIFICA 3: Adesso frame_paths è una lista di tuple: (frame_path, time_str)
    ...
    # --- MODIFICA 4: Scompattiamo (frame_path, frame_time_str)
    for i, (frame_path, frame_time_str) in enumerate(frame_paths):
        ...
        # Includiamo stile, richieste aggiuntive e il tempo corrispondente
        frame_user_text = (
            "Analizza il frame seguente. Tieni conto delle descrizioni precedenti. "
            f"\nQuesto frame è stato estratto al tempo (mm:ss): {frame_time_str}."
        )
...
with st.form(key="video_analysis_form"):
    ...
    num_frames = st.number_input("Numero di frame da estrarre (lascia vuoto se vuoi usare frame_rate)", min_value=1,
                                 value=5)
...
if start_button:
    if uploaded_video is None:
        ...
        gen = analyze_video_generator(video_data, actual_num_frames, actual_frame_rate, width, height, length_style,
                                      additional_request)
        for step_log in gen:
